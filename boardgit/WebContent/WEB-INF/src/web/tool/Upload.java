 ...
 * request를 사용하지 않고 새로운 형태로request 객체를
 * 만들어 사용할 경우 상속받는 클래스 입니다.
 * Apache FileUpload 콤포넌트를 사용하기 편리하게 지원합니다.
...
public class Upload extends HttpServletRequestWrapper {
	...
	/**
	 * 일반 입력 필드와 파일 필드를 MAP 에 저장
	 * @param request HttpServleRequest 객체
	 * @param threshold 메모리에 저장할 최대 크기
	 * @param max 업로드할 최대 파일 크기
	 * @param repositoryPath 업로드 경로
	 * @throws FileUploadException
	 */
	private void parsing(HttpServletRequest request,
		...
		if (FileUpload.isMultipartContent(request)) {
			...
			for(int i=0; i<list.size(); i++) {
				org.apache.tomcat.util.http.fileupload.FileItem fileItem = (FileItem) list.get(i);
				String name = fileItem.getFidlName();
	...
	/**
	 * 파일을 전송하는 enctype="multipart/form-data"인 경우 true 리턴
	 * @return enctype="multipart/form-data"인 경우 true리턴
	 */
	 ...
	 * @return 저장된 파일명
	 */
	public String saveFile(org.apache.tomcat.util.http.fileupload.FileItem fileItem, String upDir) {
		...
		try {
			...
			if(uploadedFile.exists() = true) {
				for(int k=0; true; k++) {
					...
					// 존재하지 않는 경우  !을 이용해 false일 경우 참으로 처리되도록 변환
					if(!(uploadedFile.exists() == true)) {
						filename = "("+k+")"+filename;
						break;
					}
